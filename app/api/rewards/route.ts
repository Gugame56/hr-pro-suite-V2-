...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Reward added successfully' });
  } catch (error) {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: 'Reward ID is required' }, { status: 400 });
