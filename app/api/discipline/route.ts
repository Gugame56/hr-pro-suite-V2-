...
export async function GET(request: Request) {
  try {
    ...
    if (employeeId) {
      const filtered = data.filter(row => row.employeeId === employeeId);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Discipline record added successfully' });
  } catch (error) {
    console.error('API Error (POST):', error);
    return NextResponse.json({ error: 'Failed to add discipline record' }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Discipline record updated successfully' });
  } catch (error) {
