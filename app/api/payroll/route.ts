...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Payroll record added successfully' });
  } catch (error) {
    console.error('API Error (POST):', error);
    return NextResponse.json({ error: 'Failed to add payroll record' }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Payroll record updated successfully' });
  } catch (error) {
